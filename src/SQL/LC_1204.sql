...
turn determines the order of which the people will board the bus, where turn=1 denotes the first person 
...
FROM (
    SELECT
        ...
        ROW_NUMBER() OVER (ORDER BY SUM(q2.weight) DESC) AS rn
    FROM Queue q1
    JOIN Queue q2
      ON q1.turn >= q2.turn
